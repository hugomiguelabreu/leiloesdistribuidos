...
public class CentroDeLeilaoDistribuido implements Serializable
{
    ...
    private Map<Integer,Leilao> leiloes; // Contem todos os leiloes
    ...
    public CentroDeLeilaoDistribuido( Map<Integer,Leilao> l){
        leiloes = l;
    }
     ...
     * 
     */
    public void licitar(int idLeilao, double valor){
        if( leiloes.containsKey(idLeilao) ){
          Leilao l = leiloes.get(idLeilao);
          synchronized(l){
                l.licitar(valor);// Pode dar-se mais coisas, do tipo
    ...
    public int createLeilao(String descricao, float valorInicial,Utilizador vendedor){
        int id = CompleteId.getNextId();
        //registarLeilao(new Leilao(descricao,valorInicial,vendedor,id));
        return id;
    }
     ...
     */
    private Map<Integer,Leilao> leiloesEmCurso(){
        Map<Integer,Leilao> res = new HashMap<>(leiloes.values()
                                                      .stream()
                                                      .filter(l-> l.isAtivo())
    ...
    public List<String> verLeiloes(Utilizador user){
        ...
        Map<Integer,Leilao> ativos = this.leiloesEmCurso();
        String header = null;
        List<String> res = new Vector<>();
        // Verificar o tipo de cliente
        boolean vendedor = user.isVendedor();
        ...
        for(Leilao leilao : ativos.values()){
            if(vendedor){
                ...
                synchronized(leilao){
                    if( leilao.getNomeMaiorLicitador().equals(user.getNome()))
                                    res.add(leilao.toString() + header);
